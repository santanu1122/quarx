...
<div class="device">
    ...
    <div class="raw100 mHide">
        ...
        <div class="raw12 tHide"><p><b>Date</b></p></div>
    ...
    <div class="raw100">
        <div class="raw25 infoBlock"><a href="<?php echo site_url('blog/editor/'.encrypt($entry->blog_id)); ?>"><?php echo valTrim($entry->blog_title, 20); ?></a></div>
        <div class="raw12 infoBlock mHide tHide"><p><?php echo valCheck($entry->blog_date); ?></p></div>
        <div class="raw13 infoBlock mHide"><p><?php echo valCheck(getCatName($entry->blog_cat)); ?></p></div>

        <div class="raw17 mHide"><button data-theme="c" onclick="window.location='<?php echo site_url('blog/editor/'.encrypt($entry->blog_id)); ?>'">Edit</button></div>
        <?php if($entry->blog_hide === '1'){ ?>
        <div class="raw16 mHide"><button data-theme="c" onclick="displayEntry('<?php echo encrypt($entry->blog_id); ?>')">Display</button></div>
        <?php }else{ ?>
        <div class="raw16 mHide"><button data-theme="b" onclick="archiveEntry('<?php echo encrypt($entry->blog_id); ?>')">Archive</button></div>
        <?php } ?>
        <div class="raw16 mHide"><button data-theme="e" onclick="deleteConfirm('<?php echo encrypt($entry->blog_id); ?>')">Delete</button></div>
    </div>
