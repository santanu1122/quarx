...
/*
    Filename:   model_event_categories.php
    Location:   /application/models/
*/

class model_event_categories extends CI_Model {
...
***************************************************************/
    ...
    function get_all_cats() {
        $qry = $this->db->query('SELECT * FROM `event_categories`');        
        if($qry){
            return $qry->result();
        }
    }
    ...
    function check_title($name, $type) {
        $qry = $this->db->query('SELECT * FROM `event_categories` WHERE cat_title = "'.$name.'"');        
        if($qry->num_rows() > 0){
            echo 'fail';
        }
    }
...
***************************************************************/
    ...
    function delete_category($id) {
        $qry = $this->db->query("SELECT * FROM events WHERE event_cat = ".$id);
        if($qry->num_rows() > 0){
            return false;
        }else{
            $cat_qry = $this->db->query("SELECT * FROM event_categories WHERE cat_parent = ".$id);
