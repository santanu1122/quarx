...
<div class="device">
    ...
    <div class="raw100">
        ...
        <div class="raw12"><p><b>Start Date</b></p></div>
    ...
    <div class="raw100">
        <div class="raw25 infoBlock"><p><?php echo valTrim($entry->event_title, 20); ?></p></div>
        <div class="raw12 infoBlock"><p><?php echo valCheck($entry->event_start_date); ?></p></div>
        <div class="raw13 infoBlock"><p><?php echo valCheck(getCatName($entry->event_cat)); ?></p></div>

        <div class="raw17"><button data-theme="c" onclick="window.location='<?php echo site_url('events/editor/'.encrypt($entry->event_id)); ?>'">Edit</button></div>
        <?php if($entry->event_hide === '1'){ ?>
        <div class="raw16"><button data-theme="c" onclick="displayEntry('<?php echo encrypt($entry->event_id); ?>')">Display</button></div>
        <?php }else{ ?>
        <div class="raw16"><button data-theme="b" onclick="archiveEntry('<?php echo encrypt($entry->event_id); ?>')">Archive</button></div>
        <?php } ?>
        <div class="raw16"><button data-theme="e" onclick="deleteConfirm('<?php echo encrypt($entry->event_id); ?>')">Delete</button></div>
    </div>
